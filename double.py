...
# Loop through the range of styles and onbet_ values to download each image
for style_number in range(2, 11):  # Adjust the range as needed
    for onbet_number in range(1, 11):  # Adjust the range as needed
        url = f"{base_url}style_{style_number}/{image_name.format(onbet_number)}"
        save_path = os.path.join(temp_dir, f"style_{style_number}_onbet_{onbet_number}.png")
